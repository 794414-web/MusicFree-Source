...
import { useFloatingWindow } from "@/core/floatingWindow/useFloatingWindow";
import { useNezhaCarAdapter } from "@/core/nezha/useNezhaCarAdapter";
...
export function BootstrapComponent() {
    ...
    // 哪吒车机适配：主题跟随、方向盘控制、多屏检测
    useNezhaCarAdapter();

    // 悬浮窗：仅当 bootstrap 完成后才真正启用，避免初始化阶段冲突
    const floatingWindowEnabled = useAppConfig("basic.floatingWindow");
    useFloatingWindow(
        bootstrapState.state === "Done" && !!floatingWindowEnabled,
    );
