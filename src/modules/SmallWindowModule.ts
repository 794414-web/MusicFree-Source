...
export const SmallWindowModule = {
   ...
   * 设置小窗口尺寸
   */
    setSmallWindowSize: async (_width: number, _height: number): Promise<boolean> => {
        return false; // 小窗口模式已禁用
    },
   ...
   * 设置小窗口位置
   */
    setSmallWindowPosition: async (_x: number, _y: number): Promise<boolean> => {
        return false; // 小窗口模式已禁用
    },
...
export const SmallWindowEvents = {
   ...
   * 监听小窗口状态变化
   */
    onSmallWindowChanged: (_callback: (isInSmallWindow: boolean) => void) => {
    // 小窗口模式已禁用，返回空监听器
        return {
            remove: () => {},
        };
   ...
   * 监听小窗口尺寸变化
   */
    onSmallWindowSizeChanged: (_callback: (width: number, height: number) => void) => {
    // 小窗口模式已禁用，返回空监听器
        return {
            remove: () => {},
        };
   ...
   * 监听小窗口位置变化
   */
    onSmallWindowPositionChanged: (_callback: (x: number, y: number) => void) => {
    // 小窗口模式已禁用，返回空监听器
        return {
            remove: () => {},
        };
