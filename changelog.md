`2026.9.23 v2.1.7`
1. 【修复】车机适配：所有面板右上角不再放「确认」按钮，避免车机屏幕右上角被遮挡导致点不到
2. 【优化】新建歌单、关联歌词、颜色选择：确认按钮内联到粘贴/输入框旁边
3. 【优化】自定义变量、歌词偏移：确认按钮移到底部操作栏（取消/确认）

`2026.9.23 v2.1.6`
1. 【修复】QQ 分享链接解析：兼容 i.y.qq.com / i2.y.qq.com 等所有 iN 子域（/n2/m/share/、/n3/other/pages/details/playlist.html?id=xxx）
2. 【增强】新增 PC 分享链接兜底：y.qq.com/portal/playlist.html?list=xxx / ?g_id=xxx / ?t=xxx
3. 【增强】歌单导入解析入口增加诊断日志，失败时写盘便于用户自查
4. 【修复】errorLog 序列化：日志里的 message 不再显示为空 {}，插件 console.warn/error 自动落盘
5. 【修复】QQ 歌单双通道：GD 聚合优先、QQ 官方兜底（60s 硬超时），避免 QQ 官方接口在部分 Android 环境挂起卡死
6. 【插件】GD 插件升级至 1.8.0，内置插件机制版本升级到 25

`2026.9.20 v2.1.5`
1. 【修复】QQ 歌单在真机上导入报「链接有误或目标歌单为空」的问题
2. 【修复】QQ 歌单拉取改为 GET 优先（手动编码 URL、仅带 UA），规避 RN 网络栈对 POST JSON body 的处理差异；POST 降为兜底通道
3. 【插件】GD 插件升级至 1.7.0
4. 【启动】内置插件机制版本升级到 17，安装后自动重装 GD 插件

`2026.9.20 v2.1.4`
1. 【修复】部分 QQ 音乐歌单（y.qq.com/n/ryqq_v2/playlist/xxx 分享链接）导入失败
2. 【修复】QQ 歌单接口返回 text/plain 未自动解析时手动解析，避免拿到空列表
3. 【增强】QQ 歌单 POST 通道失败时自动降级 GET 通道，提升导入成功率
4. 【插件】GD 插件升级至 1.6.0
5. 【启动】内置插件机制版本升级到 16，安装后自动重装 GD 插件

`2026.9.20 v2.1.3`
1. 【修复】GD 接口强制校验 User-Agent，缺失时返回 503 导致所有功能不可用
2. 【修复】歌单/榜单请求 count 参数超出 GD 上限（100→99），导致拉取失败
3. 【增强】GD 接口多域名故障转移，主域名不可用时自动切备用域名
4. 【增强】503/502/504/521 等临时服务错误纳入指数退避重试
5. 【修复】榜单/歌单详情失败时保留条目信息，避免 UI 崩溃
6. 【插件】GD 插件升级至 1.5.0
7. 【启动】内置插件机制版本升级到 15，安装后自动重装 GD 插件

`2026.9.19 v2.1.2`
1. 【插件】GD 新增榜单入口：热歌榜 / 新歌榜 / 飙升榜 / 原创榜 / 说唱榜 / 电音榜 / ACG榜 / 古典榜
2. 【插件】新增歌单详情分页浏览（getMusicSheetInfo）与榜单详情分页浏览（getTopListDetail）
3. 【插件】GD 插件升级至 1.4.0
4. 【启动】内置插件机制版本升级到 14，安装后自动重装 GD 插件

`2026.9.19 v2.1.1`
1. 【插件】GD 接口命中频率限制（HTTP 429）时按指数退避自动重试（GD 插件升级至 1.3.0）
2. 【启动】内置插件机制版本升级，安装后自动重装 GD 插件并清理旧缓存
3. 【构建】`package.json` / `package-lock.json` 版本号统一升级到 2.1.1
4. 【其他】清理代码库遗留的 TODO/FIXME 注释
5. 【其他】`mcp-server` 加启动互斥提示与协议注释，避免与本地 fake_llm 冲突

`2026.9.19 v2.1.0`
1. 【插件】内置插件精简为 GD 音乐台单一音源，订阅地址已清空
2. 【插件】修复 QQ 歌单导入失败的问题（`disstid` 必须为数字类型）
3. 【插件】繁体歌手名（如「周杰倫」）自动归一为简体，命中原唱
4. 【插件】「歌名+歌手」匹配不到时，回退到仅用歌名搜索并排除翻唱/伴奏
5. 【插件】`buildSourceCandidates` 跳过 `qqmeta` 假源，减少无效请求
6. 【插件】`getLyric` 支持返回翻译文本（GD 的 `tlyric` 字段）
7. 【插件】GD 接口命中频率限制（HTTP 429）时按指数退避自动重试
8. 【插件】QQ 歌单导入改用 `songnum` 判断翻页结束，避免多余请求
9. 【启动】修复启动初始化、远程连接和更新安装链路的若干稳定性问题
10. 【构建】`package-lock.json` 与 `package.json` 版本号对齐
11. 【其他】使用 Debug 构建通道发布本版本
